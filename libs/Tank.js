...
const { IdCharacteristic } = require('@toio/cube/lib/characteristics');

let count = 0;

let cube = null;

async function cube_connect() {
...
module.exports = class Tank extends GameObject {
    ...
    update(fDeltaTime, rectField, setWall) {
        ...
        if (bDrived) {	// 動きがある場合は、不可侵領域との衝突のチェック
            let bCollision = false;
            if (!OverlapTester.pointInRect(rectField, { fX: this.fX, fY: this.fY })) {	// フィールドの外に出た。
                bCollision = true;
            }
            else if (this.overlapWalls(setWall)) {	// 壁に当たった。
                bCollision = true;
            }
            if (bCollision) {	// 衝突する場合は、移動できない。
        ...
        if (this.objMovement['left']) {	// 左転回
            ...
            cube.move(-12.73, 12.73, 100);//14.2
    ...
    // ショット可能かどうか
    canShoot() {
        if (GameSettings.TANK_WAIT_FOR_NEW_BULLET > Date.now() - this.iTimeLastShoot) {	// ショット待ち時間内はショット不可
            return false;
        }
    ...
    // ショット
    shoot() {
        if (!this.canShoot()) {   // ショット不可の場合は、nullを返す
            return null;
        }
